...
import {
    FaUserCog, FaCheckDouble, FaClock, FaIndustry, FaFile,
    FaExclamationTriangle, FaArrowLeft, FaKey, FaRegCircle, FaCheck, FaPlay
} from "react-icons/fa";
import { AnimatedCircleCheck, AnimatedBadgeAlert } from "../components/icons/NewAnimatedIcons";
import { StaticWifi, StaticFileStack } from "../components/icons/StaticIcons"
...
import type { TestTicket, User, ApiErrorResponse } from "../types";
import { formatDateTime } from "../helpers/date";
import { useColorModeValue } from "../hooks/useColorModeValue";
import { Alert } from "../services/alertService";
import type { SweetAlertResult } from "sweetalert2";

export const TestTicketsList = () => {
    ...
    const fetchTickets = useCallback(async () => {
        try {
            ...
            const { data } = await api.get<TestTicket[]>(API_ENDPOINTS.TEST.GET_ALL);
            setTickets(data);
        } catch (error) {
    ...
    const handleConfirmStart = async () => {
        ...
        try {
            await api.post(
                API_ENDPOINTS.TEST.START(selectedTicketId),
                {
    ...
    const handleResolveTicket = async (ticketId: number) => {
        ...
        Alert.confirm("Finalizar Atendimento?", "Deseja marcar este ticket de teste como resolvido?", isDarkMode)
            .then(async (result: SweetAlertResult) => {
                if (result.isConfirmed) {
                    setIsSubmitting(true);
                    try {
                        await api.post(
                            API_ENDPOINTS.TEST.RESOLVE(ticketId),
                            {
                                resolverId: currentUser.id,
                                resolverName: currentUser.name,
                                token: ""
    ...
    const filteredTickets = tickets.filter(ticket => {
        ...
        if (filterTypes.length > 0 && !filterTypes.includes(ticket.testType)) return false;
    ...
    const getTypeBadge = (type: number) => {
        ...
        if (type === 1) return (
            <Badge {...badgeProps} color={department.log?.text} borderColor={department.log?.border}>
                <Box mr={2} display="flex" alignItems="center">
                    <StaticFileStack size={14} color="currentColor" />
                </Box>
                LOG
            </Badge>
        );
        return (
            <Badge {...badgeProps} color={department.main.text} borderColor={department.main.border}>
                <Box mr={2} display="flex" alignItems="center">
                    <StaticWifi size={14} color="currentColor" />
                </Box>
                TESTE
            </Badge>
        );
    ...
    const typeOptions = [
        { value: 0, label: "TESTE", color: "green", icon: FaIndustry },
        { value: 1, label: "LOG", color: "yellow", icon: FaFile }
    ];
    ...
    const getCanStartTicket = () => {
        ...
        if (roles.includes('test-agent')) return true;
        ...
        <Box p={{ base: 4, md: 8 }} minH="100vh" display="flex" flexDirection="column">
            <Flex align="flex-start" justify="space-between" mb={8} direction={{ base: "column", md: "row" }} gap={4}>
                <HStack align="center" h="40px">
                    ...
                    <Heading size="lg" color={labelColor}>Tickets Teste: Visão Geral</Heading>
                </HStack>
                <Flex direction="column" align="flex-end" w={{ base: "full", md: "auto" }}>
                    <TicketFilters
                        ...
                        typeOptions={typeOptions} statusOptions={statusOptions} myTicketsColor="green"
                ...
                {isFirstLoad ? (
                    ...
                    >
                        ...
                        >
                            ...
                            Nenhum ticket de teste encontrado
                        </Heading>
                    ...
                    <>
                        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={6}>
                            {currentTickets.map((ticket) => {
                                const displayToken = ticket.confirmationToken || ticket.ConfirmationToken;
                                return (
                                    <Box key={ticket.id} bg={cardBg} p={3} borderRadius="xl" borderWidth="1px" borderColor={borderColor} boxShadow="sm" transition="all 0.2s" _hover={{ transform: "translateY(-2px)", boxShadow: "md" }} position="relative">
                                        {ticket.checklistStatus === 1 && ticket.status === 3 && (
                                            <ChakraAlert status="warning" variant="solid" borderRadius="md" mb={3} py={2}>
                                                <AlertIcon boxSize="12px" />
                                                <Text fontSize="xs" fontWeight="bold">Checklist Pendente!</Text>
                                                <Button size="xs" ml="auto" colorScheme="orange" variant="outline" bg="white" onClick={() => navigate(`/checklists?ticket=${ticket.id}`)}>Preencher</Button>
                                            </ChakraAlert>
                                        )}
                                        <Flex justify="space-between" align="start" mb={2}>
                                            <Text fontWeight="bold" fontSize="lg" mt={1} color={labelColor}>ID: {ticket.id} - {ticket.lineName}</Text>
                                            <HStack gap={2}>
                                                {displayToken && (
                                                    <Badge variant="outline" fontSize="0.9rem" px={3} py={1} borderRadius="md" borderWidth="1px" display="flex" alignItems="center" title="Seu Token de Confirmação" boxShadow="none" bg="transparent" color={department.main.text} borderColor={department.main.border}><Icon as={FaKey} mr={2} boxSize="14px" /> {displayToken}</Badge>
                                                )}
                                                {getTypeBadge(ticket.testType)}
                                            </HStack>
                                        </Flex>
                                        <VStack align="start" gap={2} mb={3} w="full">
                                            ...
                                            <Box p={2} w="full" borderRadius="md" borderWidth="1px" bg={ticket.resolverName ? department.main.bg : department.log?.bg} borderColor={ticket.resolverName ? department.main.border : department.log?.border}>
                                                <Flex align="center" justify="space-between" gap={2}><Flex align="center" gap={2} fontSize="sm" fontWeight="bold" color={ticket.resolverName ? department.main.text : department.log?.text}><Icon as={FaUserCog} /><Text>{ticket.resolverName ? `Téc: ${ticket.resolverName}` : "Aguardando Responsável..."}</Text></Flex>
                                                    {!ticket.resolverName && ticket.status === 1 && canStartTicket && (
                                                        <Button size="xs" {...actionButtonOrange} variant="solid" leftIcon={<FaPlay size="10px" />} onClick={() => handleOpenStartModal(ticket.id)}>Assumir</Button>
                                                    )}
                                                    {ticket.status === 2 && currentUser?.id === ticket.resolverId && (
                                                        <Button size="xs" colorScheme="green" variant="solid" leftIcon={<FaCheck size="10px" />} onClick={() => handleResolveTicket(ticket.id)} isLoading={isSubmitting}>Finalizar</Button>
                                                    )}</Flex>
                                            </Box>
                                            <Box bg={cardBg} p={3} borderRadius="md" w="full" borderWidth="1px" borderColor={borderColor}><Text fontSize="sm" fontStyle="italic" color={textColor}>"{ticket.observation || 'Sem observações'}"</Text></Box>
                                        </VStack>
                                        <Box mt={4} w="full">
                                            <Flex justify="space-between" align="center" mb={3}><Text fontSize="xs" fontWeight="bold" color={textColor}>Progresso</Text>{getStatusBadge(ticket.status as number)}</Flex>
            ...
            <Modal isOpen={isOpen} onClose={onClose} isCentered size="sm">
                ...
                <ModalContent bg={cardBg} borderColor={borderColor} borderWidth="1px">
                    <ModalHeader>Assumir Ticket de Teste ID {selectedTicketId}</ModalHeader>
                    <ModalCloseButton />
                    <ModalBody>
                        <VStack gap={6} py={4}>
                            <Text textAlign="center" fontSize="sm" color={textColor}>Insira o token de 4 dígitos para iniciar o atendimento de Teste/Log.</Text>
                            <HStack justify="center">
                                <PinInput size="lg" autoFocus otp value={tokenInput} onChange={(val) => setTokenInput(val)}>
                                    <PinInputField bg={inputBg} borderColor={borderColor} color={textColor} _focus={{ borderColor: department.primary }} />
                                    <PinInputField bg={inputBg} borderColor={borderColor} color={textColor} _focus={{ borderColor: department.primary }} />
                                    <PinInputField bg={inputBg} borderColor={borderColor} color={textColor} _focus={{ borderColor: department.primary }} />
                                    <PinInputField bg={inputBg} borderColor={borderColor} color={textColor} _focus={{ borderColor: department.primary }} />
                                </PinInput>
                    ...
                    <ModalFooter>
                        ...
                        <Button {...actionButtonBlue} onClick={handleConfirmStart} isLoading={isSubmitting} isDisabled={tokenInput.length < 4}>Confirmar</Button>
                    </ModalFooter>
                </ModalContent>
            </Modal>
            ...
            <Modal isOpen={isSuccessStartOpen} onClose={onSuccessStartClose} isCentered size="md">
                <ModalOverlay backdropFilter="blur(4px)" />
                <ModalContent bg={cardBg} borderColor={borderColor} borderWidth="1px">
                    <ModalHeader color={labelColor}>Ticket Assumido com Sucesso!</ModalHeader>
                    <ModalCloseButton color={labelColor} />
                    <ModalBody>
                        <VStack gap={6} py={4}>
                            <Box color="green.400"><AnimatedCircleCheck size={60} color="currentColor" /></Box>
                            <Text textAlign="center" fontSize="lg" fontWeight="bold" color={labelColor}>Ticket ID {successTicketId} em Andamento</Text>
            ...
            <Modal isOpen={isSuccessResolveOpen} onClose={onSuccessResolveClose} isCentered size="md">
                <ModalOverlay backdropFilter="blur(4px)" />
                <ModalContent bg={cardBg} borderColor={borderColor} borderWidth="1px">
                    <ModalHeader color={labelColor}>Ticket Finalizado com Sucesso!</ModalHeader>
                    <ModalCloseButton color={labelColor} />
                    <ModalBody>
                        <VStack gap={6} py={4}>
                            <Box color="green.400"><AnimatedCircleCheck size={60} color="currentColor" /></Box>
                            <Text textAlign="center" fontSize="lg" fontWeight="bold" color={labelColor}>Ticket ID {successTicketId} Resolvido</Text>
